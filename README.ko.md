...
| **Reasonix + 다른 모델 사용 중** | ⚠️ **미검증.** Reasonix는 AI SDK를 통해 다른 제공사(Anthropic, OpenAI, Google 등)도 지원하나, BlackCow는 DeepSeek에서만 테스트되었습니다. 다른 모델을 시도한다면 YAML frontmatter의 `model_tiers`를 해당 모델명으로 수정하고 컨텍스트 버짓을 조정하세요. 결과는 보장할 수 없습니다. |
| **Claude Code, Codex CLI, OpenCode 등 다른 하네스 사용 중** | ⚠️ **포팅 필요.** BKIT 방법론은 특정 하네스에 종속되지 않습니다. 현재 `.md` 스킬 파일은 Reasonix 네이티브이므로 — `task`, `edit_file`, `multi_edit` 등의 도구 호출을 해당 하네스에 맞게 재작성해야 합니다. |
