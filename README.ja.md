<div align="center">
  ...
  <p><strong>BKITにインスパイアされたエージェントエンジニアリングハーネス。</strong><br />
  Reasonix + DeepSeek向けに構築。</p>
...
| コマンド | 機能 |
| --- | --- |
| `blackcow-plan` | 戦略プランナー。コードベースを10の角度から分析し、3つのアーキテクチャオプションを提案し、決定完全な計画書を作成。製品コードは一切編集しません。 |
...
> **plan→execute→verify 1サイクルの推定コスト**: $0.03未満（DeepSeek）。これはトークンカウンティングに基づく推定であり、実測ベンチマークではありません。実際のコストはプロジェクトサイズとタスクの複雑さによって異なります。
