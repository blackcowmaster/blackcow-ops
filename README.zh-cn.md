<div align="center">
  <h1>BlackCow Ops</h1>
  <p><strong>为 Reasonix + DeepSeek 打造的 7 个自我改进工作流技能。</strong></p>
  <p>
...
BlackCow Ops 是一套 7 个 Reasonix 技能，构成编码任务的 **govern → plan → execute → verify → evolve** 流水线。强制执行 BKIT 11 门质量体系，并针对 DeepSeek 的成本优势进行了优化。修复拼写错误约 $0.001，多文件功能约 $0.03。

**诚实分数：89.8/100**（11 维度平均）。评分标准固定 — 不移动目标。
...
| `blackcow-loop` | 执行引擎。5 种模式（FAST~ESCALATE）、PDCA、Findings Gate、O0-O4 验证。 |
...
- **[BKIT by POPUP STUDIO](https://github.com/popup-studio-ai/bkit-claude-code)** — 原始 11 门质量体系和 PDCA 方法论。Apache 2.0。
- **[OmO / oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent)** — 纪律智能体、并行编排、Hashline 验证。
- **[fablize](https://github.com/fivetaku/fablize)** — "流程无法提升模型上限"的诚实限制哲学。
- **[FableCodex](https://github.com/baskduf/FableCodex)** — Findings Gate 模式（基于证据的解决、阻止未解决 finding）。
- **[prometheus](https://github.com/tmdgusya/prometheus)** — 上下文优先分解、ZCode /goal 强制执行洞察。
- **[superpowers-reasonix](https://github.com/christopherarter/superpowers-reasonix)** — 根因优先诊断纪律（Phase 2.2）。
